...
function fixture(options: { pending?: boolean; lostAck?: boolean; wrongThread?: boolean; loaded?: boolean; wrongItemTurn?: boolean; transientInterrupted?: boolean; clientMessageId?: string } = {}) {
  ...
  const client: ICodexRpcClient = {
    ...
    async request(method, params) {
      ...
      if (method === 'thread/loaded/list') return { data: options.loaded ? ['desktop'] : [] };
      ...
      if (method === 'thread/turns/list') return {
        data: [
          ...(created && !options.pending ? [
            ...
            { id: 'our-turn', status: completed ? 'completed' : 'interrupted', completedAt: completed ? 1788933273 : null, items: [{ type: 'userMessage', id: 'our-user', clientId }] },
          ] : []),
          { id: 'baseline', status: 'completed', items: [] },
        ], nextCursor: null,
      };
      if (method === 'thread/items/list') {
        assert.equal(params.turnId, 'our-turn');
        return { data: [
          { turnId: options.wrongItemTurn ? 'foreign-turn' : 'our-turn', item: { type: 'userMessage', id: 'our-user', clientId } },
          ...(completed ? [{ turnId: 'our-turn', item: { type: 'agentMessage', id: 'answer', text: 'DESKTOP_REPLY' } }] : []),
        ], nextCursor: null };
      }
...
test('an observer that loaded a writer or returns another turn cannot report success', async () => {
  for (const options of [{ loaded: true }, { wrongItemTurn: true }]) {
